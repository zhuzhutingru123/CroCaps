...
def train():
    ...
    parser.add_argument("--model_checkpoint", type=str, default="msvd_step_par0_loss12456/", help="Path, url or short name of the model")
    parser.add_argument("--log_path", type=str, default="msvd_step_par0_loss12456/", help="Log path")
    ...
    def update(engine, batch):
        ...
        for i in range(i3d.size(0)):
            x=torch.ones([31], dtype=torch.int64) * domain[i]
            label_domain_neg[i, :] = torch.cat([x, label_domain_neg[i,31:]], dim=0)
        ...
        video_embs_share = model.video_ff(i3d)
        ...
        '''
        label_domain_neg = torch.ones([i3d.size(0), labels.size(1)], dtype=torch.int64) * (-1)
        label_domain_neg_word = torch.ones([i3d.size(0), labels.size(1)], dtype=torch.int64) * (-1)
        label_confidence = torch.ones([i3d.size(0), 1], dtype=torch.float32)
        ...
        for i in range(i3d.size(0)):
                ...
                #np.random.shuffle(i3d[i,:,:])


            label_domain_neg[i,0]=domain[i]
            label_domain_neg_word[i,1:]=domain[i]
            #label_domain_neg[i, :] = domain[i]


            '''
            if domain[i]==1:
                if random.random()<0.5:
                    j=random.randint(0,i3d.size(0)-1)
                    i3d[i,:,:]=lamda_confident*i3d[i,:,:]+(1-lamda_confident)*i3d[j,:,:]
                    label_confidence[i,0]=0.5
            '''
            ...
            if domain[i] == 0:
                ...
                label = int(vid[i][3:])
                labels[i,] = torch.ones([1, labels.size(1)], dtype=torch.int64) * (-1)
                greedy_caption, lable_predict = generate_response_greedy(tokenizer, model,  i3d, vid, args)
                reward = sim(greedy_caption,i3d,model_clip)
                #print(reward[0][0].cpu().detach().numpy())
                #if label % 10 > par:
                    #labels[i,] = torch.ones([1, labels.size(1)], dtype=torch.int64) * (-1)
                    #flag =0
            else:
                ...
                #gg_caption_clip=reward[0][0].item()


        input_ids = input_ids.to(args.device)
        token_type_ids = token_type_ids.to(args.device)
        label_confidence = label_confidence.to(args.device)
        labels = labels.to(args.device)
        input_mask = input_mask.to(args.device)
        i3d = i3d.to(args.device)
        video_mask = video_mask.to(args.device)
        reply_mask = reply_mask.to(args.device)
        label_domain_neg = label_domain_neg.to(args.device)
        label_domain_neg_word = label_domain_neg_word.to(args.device)

        input_embs = model.transformer.wte(input_ids)
        video_embs = model.video_ff(i3d)


        input_embs1 = torch.cat([video_embs, input_embs], dim=1)
        token_type_ids1 = torch.cat(
            [torch.ones((i3d.size(0), i3d.size(1))).long().cuda() * tokenizer.convert_tokens_to_ids(SPECIAL_TOKENS[-2]),
             token_type_ids], dim=1)
        label_i3d=i3d

        # [8,44] qian 30 50261 50259 houmian 50262
        video_loss = model(input_embs1,
                           token_type_ids1=token_type_ids1,
                           labels1=(labels, label_i3d),
                           attention_mask1=[video_mask, input_mask], mode="video")[0]

        reply_loss = model(input_embs1,  token_type_ids1=token_type_ids1, labels1=(labels, label_i3d,label_confidence),
                           attention_mask1=[reply_mask, input_mask], mode="reply")[0]


        entropy_loss= model(input_embs1,  token_type_ids1=token_type_ids1, labels1=(labels, label_i3d),
                           attention_mask1=[reply_mask, input_mask], mode="entropy")[0]

        video_domain_loss = model(input_embs1,
                           token_type_ids1=token_type_ids1,
                           labels1=(labels, label_domain_neg,label_confidence),
                           attention_mask1=[video_mask, input_mask],  mode="domain")[0]


        word_domain_loss = model(input_embs1,token_type_ids1=token_type_ids1,labels1=(labels, label_domain_neg_word),
                                 attention_mask1=[video_mask, input_mask], mode="domain2")[0]
        ...
        loss3 = entropy_loss/ args.gradient_accumulation_steps
        ...
        loss5 = word_domain_loss / args.gradient_accumulation_steps



        #loss = loss2+loss1*10+loss6
        rank_loss = model(input_embs1, token_type_ids1=token_type_ids1, labels1=(labels, label_i3d, label_confidence),
                          attention_mask1=[reply_mask, input_mask], avg=avg, reward=reward, mode="rank")[0]
        loss6 = rank_loss / args.gradient_accumulation_steps
        loss = loss1 * 10 + loss6 * 10 + loss4 * 3 + loss5 * 3+loss2


        '''
        if domain[i]==0:
            if  flag == 1:
                loss = loss1 * 10 + loss2+ loss4*3 + loss5*3
            else:
                rank_loss =  model(input_embs1, token_type_ids1=token_type_ids1, labels1=(labels, label_i3d, label_confidence),
                  attention_mask1=[reply_mask, input_mask], avg=avg, reward=reward, mode="rank")[0]
                loss6 = rank_loss / args.gradient_accumulation_steps
                loss = loss1*1000+loss6*20 + loss4*3  + loss5*3

        else:
            #rank_loss = model(input_embs1, token_type_ids1=token_type_ids1, labels1=(labels, label_i3d, label_confidence),
                      #attention_mask1=[reply_mask, input_mask], avg=gg_caption_clip, reward=reward, mode="rank")[0]
            #loss6 = rank_loss / args.gradient_accumulation_steps

            loss = loss1 * 1000 + loss2 + loss4*3 + loss5*3
        '''
    ...
    def inference(engine, batch):
        ...
        result, all_decoded_for_eval1 = generate_response(tokenizer, model, features_rgb,  attibute_fea_msvd,test_dataset, args)
