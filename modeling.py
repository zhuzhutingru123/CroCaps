...
class CLIP4ClipPreTrainedModel(PreTrainedModel, nn.Module):
    ...
    @classmethod
    def from_pretrained(cls, cross_model_name, state_dict=None, cache_dir=None, type_vocab_size=2, device="cuda:0",*inputs, **kwargs):
        ...
        model = cls(cross_config, clip_state_dict, device=device,*inputs, **kwargs)
...
class CLIP4Clip(CLIP4ClipPreTrainedModel):
    def __init__(self, cross_config, clip_state_dict, task_config,device="cuda:0"):
        ...
        self.clip = CLIP(
            ...
            linear_patch=self.linear_patch,device=self.device
        ).float()
    ...
    def _mean_pooling_for_similarity_sequence(self, sequence_output, attention_mask):
        attention_mask_un = attention_mask.to(dtype=torch.float).unsqueeze(-1).to(self.device)
    ...
    def _mean_pooling_for_similarity_visual(self, visual_output, video_mask,):
        video_mask_un = video_mask.to(dtype=torch.float).unsqueeze(-1).to(self.device)
